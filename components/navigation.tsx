"use client"

import { useEffect, useState } from "react"
import Link from "next/link"
import { usePathname } from "next/navigation"
import { Menu, X } from "lucide-react"
import { Button } from "@/components/ui/button"

const navItems = [
  { href: "/#uslugi", label: "Услуги" },
  { href: "/#raboty", label: "Работы" },
  { href: "/#process", label: "Процесс" },
  { href: "/#raschet", label: "Расчет" },
]

export function Navigation() {
  const [isOpen, setIsOpen] = useState(false)
  const [isScrolled, setIsScrolled] = useState(false)
  const pathname = usePathname()
  const isHomepage = pathname === "/"
  const showSolid = isHomepage ? isScrolled : !isScrolled

  useEffect(() => {
    const handleScroll = () => setIsScrolled(window.scrollY > 50)
    window.addEventListener("scroll", handleScroll, { passive: true })
    handleScroll()
    return () => window.removeEventListener("scroll", handleScroll)
  }, [])

  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-5xl">
      <div
        className={`rounded-full flex items-center justify-between shadow-xl transition-all duration-300 ease-out ${
          showSolid ? "bg-[#1a2234]/95 backdrop-blur-md px-6 py-2.5" : "bg-white/10 backdrop-blur-sm border border-white/20 px-6 py-3"
        }`}
        style={{ transform: showSolid ? "scaleY(0.95)" : "scaleY(1)", transformOrigin: "center center" }}
      >
        <Link href="/" className="flex items-center">
          <span className="text-white font-bold text-lg tracking-tight">
            <span className="text-white/90">Art</span>
            <span className="text-white/70">Dom</span>
          </span>
        </Link>

        <div className="hidden md:flex items-center gap-6">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`text-sm transition-all duration-300 ${
                showSolid ? "text-white/70 hover:text-white" : "text-white/90 hover:text-white"
              }`}
            >
              {item.label}
            </Link>
          ))}
        </div>

        <div className="flex items-center gap-3">
          <a
            href="https://wa.me/79990000000"
            target="_blank"
            rel="noopener noreferrer"
            className="bg-[#22c55e] hover:bg-[#16a34a] p-2.5 rounded-full transition-all duration-300"
            aria-label="Написать в WhatsApp"
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5 text-white fill-current">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
            </svg>
          </a>

          <Button asChild className="hidden sm:flex rounded-full px-5 py-2 text-sm font-medium bg-white/20 border border-white/30 hover:bg-white/30 text-white">
            <Link href="/#raschet">Отправить фото</Link>
          </Button>

          <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-white p-2" aria-label="Меню">
            {isOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </button>
        </div>
      </div>

      <div
        className={`md:hidden mt-2 rounded-2xl p-4 shadow-xl overflow-hidden transition-all duration-300 ease-out ${
          showSolid ? "bg-[#1a2234]/95" : "bg-white/10 border border-white/20"
        } backdrop-blur-md ${isOpen ? "opacity-100 max-h-96 translate-y-0" : "opacity-0 max-h-0 -translate-y-2 pointer-events-none"}`}
      >
        <div className="flex flex-col gap-3">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              onClick={() => setIsOpen(false)}
              className="text-white/80 hover:text-white text-sm py-2 transition-colors"
            >
              {item.label}
            </Link>
          ))}
          <Button asChild className="w-full rounded-full mt-2 bg-white/15 hover:bg-white/25 text-white">
            <Link href="/#raschet" onClick={() => setIsOpen(false)}>
              Отправить фото
            </Link>
          </Button>
        </div>
      </div>
    </nav>
  )
}
