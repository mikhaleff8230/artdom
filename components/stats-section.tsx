...
import type { StatsSectionContent } from "@/lib/sanity"
...
export function StatsSection({ content }: StatsSectionProps) {
  const badge = content?.badge ?? "Почему выбирают нас"
  const title = content?.title ?? "Отвечаем за объект от первого осмотра до сдачи"
  ...
  return (
    <section className="py-20 bg-[#0f1629]">
      <div className="container mx-auto px-4">
        ...
        <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white text-center mb-16">{title}</h2>
